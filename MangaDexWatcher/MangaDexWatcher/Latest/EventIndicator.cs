...
/// <summary>
/// Represents the return results of the <see cref="ILatestChaptersService"/> that indactes some resolution
/// </summary>
...
public class ErrorIndicator : IEventIndicator
{
    ...
    public string Title => Context?.Attributes?.Title ?? Context?.Attributes?.Chapter ?? Context?.Id?.ToString() ?? "Unknown";

    public string Id => Context?.Id?.ToString() ?? "Unknown";
